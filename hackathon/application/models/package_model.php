...
class Package_model extends CI_Model {
    ...
    function get_record($key){
        $query = $this->db->get_where('tbl_vender_packages', array('id' => $key))->result_array();
	if (count($query) == 0)
    ...
    function view_records(){
        ...
        $userid = $var['id'];
        $query = $this->db->query('SELECT * FROM tbl_vender_packages WHERE int_vender_id='.$userid.' AND intEnable=1')->result_array();	
        return $query;
    }
	...
	/*$config['upload_path'] = 'http://localhost:82/hackathon/';
        ...
        $var = $this->session->userdata['user'];
        $data_arry = $_POST;
        $data_arry['int_vender_id']= $var['id'];
	...
	$this->load->library('upload', array('upload_path' => './resources/images/user_uploads/', 'allowed_types' => 'gif|jpg|png'));        
        ...
        $this->db->insert('tbl_vender_packages', $data_arry);
	$this->session->set_userdata('success', 'New Package Added');
	return true;
    }
	...
	$this->load->library('upload', array('upload_path' => './resources/images/user_uploads/', 'allowed_types' => 'gif|jpg|png'));        
        ...
        $this->db->update('tbl_vender_packages', $data_arry, array('id' => $this->input->post('id')));
	$this->session->set_userdata('success', 'Package Data Updated');
	return true;
    }
    
    function delete_entry($id){
	
        $this->db->update('tbl_vender_packages', array('intEnable' => 0), array('id' => $id));
	$this->session->set_userdata('success', 'Package Deleted Successfully');
	return true;
    }
    
    function delete_entry_test($id){
